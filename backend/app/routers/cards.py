...
async def create_card(
    ...
    # board_id is already known from the card's column — no extra query needed
    from app.models.column import Column
    from sqlalchemy import select
    col_result = await db.execute(select(Column).where(Column.id == column_id))
    col = col_result.scalar_one_or_none()
    board_id = col.board_id if col else ""

    card_out = CardOut.model_validate(card)
    await sio.emit("card_created", {"card": card_out.model_dump(mode="json")}, room=board_id)
    await _emit_activity(board_id, current_user, f"{current_user.display_name} created card '{card.title}'", db)

    if card.assigned_user_id and card.assigned_user:
        background_tasks.add_task(
            email_service.send_assignment_email,
            card.assigned_user.email, card.title, board_id, current_user.display_name,
        )
...
async def update_card(
    ...
    card = await card_service.update_card(card_id, data, current_user.id, db)
    ...
    await _emit_activity(board_id, current_user, f"{current_user.display_name} updated card '{card.title}'", db)
...
async def delete_card(
    ...
    card_data = await card_service.delete_card(card_id, current_user.id, db)

    from app.models.column import Column
    from sqlalchemy import select
    col_result = await db.execute(select(Column).where(Column.id == card_data["column_id"]))
    col = col_result.scalar_one_or_none()
    board_id = col.board_id if col else ""

    await sio.emit("card_deleted", {"card_id": card_data["id"], "column_id": card_data["column_id"]}, room=board_id)
    await _emit_activity(board_id, current_user, f"{current_user.display_name} deleted card '{card_data['title']}'", db)
...
async def move_card(
    ...
    card = await card_service.move_card(card_id, data.column_id, current_user.id, db)
    ...
    await _emit_activity(board_id, current_user, f"{current_user.display_name} moved card '{card.title}'", db)
